...
-- 1) Podrobn� preh�ad o vy�etrovan�ch pr�padoch.
...
-- 3) �tatistika trestn�ch �inov pod�a regi�nov, miest, obvodov.
--podla regionov
...
-- 4) �tatistika (po�ty) trestn�ch �inov r�znych druhov, zoraden� od naj�astej�ie sa vyskytuj�ceho druhu po najzriedkavej�ie.
...
-- 5) �tatistick�  vyhodnotenie sp�sobenej �kody pod�a r�znych krit�ri�.
--podla regionov
select reg.nazov as region, 
...
--6) Miera objasnenosti trestn�ch �inov v jednotliv�ch regi�noch, za ur�it� obdobie. 
...
--7) Miera objasnenosti naj�astej�ie sa vyskytuj�ceho druhu trestn�ch �inov.
...
--8) Zoznam os�b u ktor�ch sa m��e bli��ie posudzova� n�rok na amnestiu udelen� prezidentom pri pr�le�itosti 20. v�ro�ia 
--   zalo�enia nemenovan�ho �t�tu (krit�ria si vymyslite).
...
where (months_between(sysdate,add_months(odo.dat_nastupu, odo.dlzka_trestu*12)) < 0)
    ...
    and (tp.druh_pripadu in ('usmrtenie','ubl�enie na zdrav�', 'podpl�canie'))
    and (odo.dlzka_trestu < 10);
    
--9) Dlhodob� �tatistika po�tu os�b vo v�kone trestu (po�et za rok). 
...
--10) Ro�n� n�klady na mzdy pracovn�kov PZ
